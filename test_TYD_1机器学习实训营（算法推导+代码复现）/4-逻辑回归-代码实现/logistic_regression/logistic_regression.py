...
class LogisticRegression:
    ...
    def train(self,max_iterations=1000):
        cost_histories = []
        num_features = self.data.shape[1]
        for label_index,unique_label in enumerate(self.unique_labels):  #对每个类别分别进行训练
    ...
    def predict(self,data):
        ...
        prob = LogisticRegression.hypothesis(data_processed,self.theta.T)  #预测的概率值
        max_prob_index = np.argmax(prob, axis=1)        # 找出每个样本当前最大的概率值索引
        class_prediction = np.empty(max_prob_index.shape,dtype=object)  #初始化类别
        for index,label in enumerate(self.unique_labels):
            class_prediction[max_prob_index == index] = label # 将最大概率值索引对应的类别赋给class_prediction
